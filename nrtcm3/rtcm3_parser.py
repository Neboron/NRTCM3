...
class rtcm3_parser:
    ...
    def process_rtcm_data(self, data_buffer):
        if len (data_buffer) <  RTCM3_Min_Size :
            log("TOO SHORT", "red")
            return

        if data_buffer[0] <> RTCM3_Preamble : # {It isn't a valid packet, skip to first start}
            for i  in range (0,len(data_buffer)):
                log("in Did not get a STX: " + str(i), "red")
                if (data_buffer[0] != RTCM3_Preamble) :
                    self.undecoded.append(data_buffer[0]);
                    del data_buffer[0];
                else:
                    break;
            return Got_Undecoded

        self.packet_length = (data_buffer[RTCM3_Length_Location] & 0x03);
        self.packet_length = self.packet_length << 8;
        self.packet_length = self.packet_length | data_buffer[RTCM3_Length_Location+1];
        log("Packet Length: {:X}".format(self.packet_length), "yellow")

        if (self.packet_length+RTCM3_Min_Size) > len(data_buffer):
            return Need_More

        Computed_CRC = crc_normal(data_buffer[0:self.packet_length + RTCM3_First_Data_Location]);
        log("Computed CRC {:X}".format(Computed_CRC), "yellow")
        CRC = data_buffer[RTCM3_First_Data_Location + self.packet_length];
        CRC = CRC << 8;
        CRC = CRC | data_buffer[RTCM3_First_Data_Location + self.packet_length+1];
        CRC = CRC << 8;
        CRC = CRC | data_buffer[RTCM3_First_Data_Location + self.packet_length+2];
        log("CRC {:X}".format(CRC), "yellow")

        if CRC == Computed_CRC:
            self.packet_ID = data_buffer[RTCM3_First_Data_Location];
            self.packet_ID = self.packet_ID << 4;
            self.packet_ID = self.packet_ID | ((data_buffer[RTCM3_First_Data_Location+1] >> 4) & 0x0F);
            log("Packet_ID" + str(self.packet_ID), "yellow")
            self.packet = data_buffer[:self.packet_length + RTCM3_Min_Size]
            packet_data=self.packet[RTCM3_First_Data_Location:self.packet_length+RTCM3_First_Data_Location] # The packet
            data_buffer=data_buffer[self.packet_length + RTCM3_Min_Size:]
